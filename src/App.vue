    ...
    <div class="container">
      ...
      <header class="header-manifest">
        <div class="header-inner">
           <button @click="cycleTheme" class="main-ctrl-btn theme-pos" title="Сменить тему">
          ...
          <div class="sticky-nav-group">
              ...
              <div class="ctrl-wrapper-desktop">
                ...
                <div class="left-group">
                  <div class="control-item">
                     <button @click="showDash = !showDash" :class="['main-ctrl-btn', { 'active-mode': showDash }]" title="Статистика">
                       <span class="rus-font ctrl-text-bold" style="letter-spacing: 0.5px;">Дашборд</span>
                       <svg :class="['pill-arrow', { 'closed-arrow': !showDash }]" viewBox="0 0 24 24"><path fill="currentColor" d="M7.41 8.59L12 13.17L16.59 8.59L18 10L12 16L6 10L7.41 8.59Z"/></svg>
                     </button>
                  </div>
                  ...
                  <div class="control-item relative-zone">
                     ...
                     <transition name="pop">
                        <div v-if="showBrandMenu" class="bahur-popup-menu list-mode liquid-glass">
                  ...
                  <div class="control-item relative-zone">
                     ...
                     <transition name="pop">
                        <div v-if="showAromaMenu" class="bahur-popup-menu list-mode liquid-glass">
                           <div class="search-input-box">
                  ...
                  <div class="control-item relative-zone">
                    ...
                    <transition name="pop">
                       <div v-if="showUpdatesMenu" class="bahur-popup-menu list-mode liquid-glass">
                ...
                <div class="right-group">
                  <div class="control-item relative-zone">
                    <button @click="toggleFilterMenu" :class="['main-ctrl-btn', { 'active-mode': showFilters }]">
                     ...
                     <transition name="pop">
                      <div v-if="showFilters" class="bahur-popup-menu filter-mode liquid-glass">
            ...
            <div class="grid-layout-def head no-click">
              <div class="cell id head-txt center tooltip-wrapper">
                 ...
                 <div class="legend-tooltip liquid-glass">
          ...
          <div class="grid-table">
            <div v-for="(p, index) in sortedProducts" :key="p.id + '-' + index" 
              ...
              <div class="row-visual-layer">
                <div class="cell line id-zone-square center">
                  <div class="id-sq-top mono">{{ p.id }}</div>
                  <div :class="['lamp-sq', p.lampColor]"></div>
                </div>
                <div class="cell line name border-right-mobile">
                ...
                <div class="cell line desk-only center"><div class="badge-square-matte">{{ getSex(p.gender) }}</div></div>
                <div class="cell line desk-only center"><div class="badge-square-matte">{{ p.factory }}</div></div>
                <div class="cell line desk-only center"><div class="badge-square-matte">{{ p.quality }}</div></div>
...
const activeUpdateFilter = ref('all'); 
...
const startPulsingFavicon = () => {
  ...
  faviconInterval = setInterval(() => {
    phase += 0.15;
    const r = 10 + Math.sin(phase) * 4; // Пульсация внешнего круга от 6 до 14
    const op = 0.3 + Math.sin(phase) * 0.5; // Пульсация прозрачности
    const color = activeTheme.value === 'light' ? '%23000000' : '%23FFFFFF';
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
       <circle cx="16" cy="16" r="${r}" fill="${color}" opacity="${op}"/>
...
/* ЦВЕТОВЫЕ ТЕМЫ С ПОЛНЫМ REDESIGN (LIQUID GLASS + ЧЕТКОСТЬ) */
.bahur-terminal {
...
/* 1. БЕЛАЯ ТЕМА (Чистый минимализм 2026) */
.bahur-terminal.light {
  --bg: #F5F5F7; 
  --text: #000000;
  --table-border: rgba(0, 0, 0, 0.25); /* Строгие и видные черные линии */
  --panel-bg: rgba(255, 255, 255, 0.65); 
  --popup-bg: rgba(255, 255, 255, 0.85); /* Отдельный слой для меню */
  --shadow-layer: 0 16px 40px rgba(0, 0, 0, 0.1);
...
/* 2. ЧЕРНАЯ ТЕМА (NOIR - Глубокие слои и Liquid Glass) */
.bahur-terminal.noir {
  --bg: #000000;
  --text: #FFFFFF;
  --table-border: rgba(255, 255, 255, 0.3); /* ОЧЕНЬ четкие белые линии */
  --panel-bg: rgba(24, 24, 27, 0.55); /* Темно-серый полупрозрачный низ */
  --popup-bg: rgba(36, 36, 42, 0.85); /* Более светлый серый для меню, чтобы не сливалось */
  --shadow-layer: 0 20px 50px rgba(0, 0, 0, 0.95); /* Мощная тень для разделения */
  --glass-border: rgba(255, 255, 255, 0.15); /* Блик стекла по краям */
  --dim: #999;
  --seg-bg: #1A1A1D; --seg-active: #FFFFFF; --seg-txt: #666; --seg-txt-active: #000000;
  --aura-bg: rgba(0, 0, 0, 0.7); --aura-text: #FFFFFF;
}

/* 3. RGB BUSINESS (Неоновый кибер-офис) */
.bahur-terminal.rgb-business {
  ...
  --shadow-layer: 0 15px 45px rgba(0, 255, 170, 0.08);
  --glass-border: rgba(56, 189, 248, 0.2);
  --dim: #64748B;
  --seg-bg: #0F172A; --seg-active: #38BDF8; --seg-txt: #64748B; --seg-txt-active: #0B0E14;
  --aura-bg: rgba(15, 23, 42, 0.8); --aura-text: #38BDF8;
}

/* Утилита для Liquid Glass */
.liquid-glass {
...
.error-box-glass { text-align: center; border: 1px solid var(--glass-border); padding: 40px 60px; border-radius: 16px; background: var(--panel-bg); backdrop-filter: blur(20px); max-width: 400px; }
...
.retry-btn-noir { background: var(--text); border: none; color: var(--bg); padding: 12px 24px; border-radius: 22px; font-family: 'JetBrains Mono', monospace; font-size: 11px; cursor: pointer; transition: 0.3s; text-transform: uppercase; font-weight: 700; }
...
.theme-pos { position: absolute; right: 0; top: 50%; transform: translateY(-50%); min-width: auto; padding: 0 15px; }

/* STICKY NAV (Liquid Glass Panel) */
.sticky-nav-group { 
...
/* УПРАВЛЯЮЩИЕ КНОПКИ (Единый размер для всех) */
.main-ctrl-btn { 
  height: 44px; /* Жестко фиксированная высота для всех кнопок панелей */
  flex: 1; 
  min-width: 140px; 
...
.pill-arrow { width: 10px; height: 10px; opacity: 0.8; transition: transform 0.3s; } 
.closed-arrow { transform: rotate(180deg); }
...
/* DASHBOARD */
.dash-collapsible-wrapper { display: grid; grid-template-rows: 0fr; transition: grid-template-rows 0.3s cubic-bezier(0.4, 0, 0.2, 1); margin-bottom: 15px; }
.dash-collapsible-wrapper.open { grid-template-rows: 1fr; }
.dash-inner-content { overflow: hidden; padding-top: 5px; }
.dash-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 10px; }
.stat-card { border: 1px solid var(--table-border); padding: 18px; background: var(--panel-bg); border-left: 3.5px solid var(--text); border-radius: 12px; backdrop-filter: blur(10px); }
...
/* ТАБЛИЦА-КАРТОЧКИ С ЧЕТКИМИ ЛИНИЯМИ */
...
/* Дизайн карточек с строгими линиями */
.card-row {
  background: var(--bg);
  border-radius: 12px;
  margin-bottom: 6px;
  border: 1px solid var(--table-border); /* Четкий бордер всей строки */
  box-shadow: 0 4px 6px rgba(0,0,0,0.1);
  overflow: hidden; position: relative;
}
.cell { height: auto; display: flex; align-items: center; padding: 15px; box-sizing: border-box; overflow: visible; }

/* Возвращены внутренние линии (разделители) для четкости! */
.cell.line { border-right: 1px solid var(--table-border); }
.cell.line:last-child { border-right: none; }
...
.clickable-row:hover .row-aura-overlay, .clickable-row.simulated-hover .row-aura-overlay { opacity: 1; background: var(--aura-bg); backdrop-filter: blur(2px); }
