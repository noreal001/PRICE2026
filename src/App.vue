                ...
                <div class="left-group">
                  ...
                  <div class="control-item relative-zone pos-center-mobile">
                    <button @click="toggleNewMenu" :class="['main-ctrl-btn', { 'active-mode': showNewMenu || filterPlus || filterStar || showOut }]">
                       <span class="ctrl-text-bold">Статус</span>
                       <svg class="pill-arrow" viewBox="0 0 24 24"><path fill="currentColor" d="M7.41 8.59L12 13.17L16.59 8.59L18 10L12 16L6 10L7.41 8.59Z"/></svg>
                    </button>
                    <transition name="pop">
                      <div v-if="showNewMenu" class="bahur-popup-menu center-mode">
                         ...
                         <div class="toggle-row" @click="filterStar = !filterStar">
                           <span class="toggle-label">Новые версии (*)</span>
...
.intro-text { font-weight: 900; font-size: 60px; color: var(--text); letter-spacing: 8px; opacity: 0; text-shadow: 0 0 15px rgba(255,255,255,0.6); animation: scale-in 1.5s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
...
.header-pill-btn {
  ...
  border-radius: 20px; padding: 6px 12px; font-size: 10px; font-weight: 800; cursor: pointer;
...
.ctrl-text-bold { font-weight: 800; }
...
.toggle-label { font-size: 10px; color: var(--text); font-weight: 700; text-transform: none; letter-spacing: 0;}
...
.popup-input { width: 100%; background: var(--seg-bg); border: 1px solid var(--border); padding: 10px; border-radius: 8px; color: var(--text); font-size: 12px; outline: none; margin-bottom: 5px; box-sizing: border-box; font-family: 'Nunito', sans-serif; font-weight: 600;}
...
.brand-row-btn { display: flex; justify-content: space-between; align-items: center; background: transparent; color: var(--text); border: none; padding: 10px; border-radius: 8px; cursor: pointer; font-size: 11px; text-transform: capitalize; text-align: left; transition: 0.2s; opacity: 0.8; }
...
.segment-btn { flex: 1; background: transparent; border: none; color: var(--seg-txt); padding: 6px 0; font-size: 10px; font-weight: 700; border-radius: 6px; cursor: pointer; transition: 0.2s; text-transform: none;}
...
.q-meta { display: flex; justify-content: space-between; font-size: 9px; font-weight: 700; margin-bottom: 5px; }
.op-5 { opacity: 0.5; }
...
.ap-item { font-size: 11px; font-weight: 600; color: var(--dim); }
.ap-item .val { color: var(--text); font-weight: 800; font-size: 12px; margin-left: 5px; }
.stock-stack-info { display: flex; flex-direction: column; margin-top: 10px; gap: 2px; }
.ss-row { font-size: 10px; color: var(--dim); font-weight: 700; }
...
.stock-missing-text { font-size: 9px; font-weight: 600; color: var(--dim); text-align: right; }
...
.header-search-input {
  ...
  font-family: 'Nunito', sans-serif; font-size: 10px; font-weight: 800; letter-spacing: 1px; text-transform: uppercase;
}
...
.header-search-container .clear-search {
  ...
  cursor: pointer; font-size: 10px; font-weight: 900; transition: 0.2s;
}
...
.row-aura-overlay { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; opacity: 0; background: transparent; transition: all 0.4s; z-index: 10; pointer-events: none; border-radius: 16px;}
...
.aura-text { font-size: 11px; font-weight: 900; letter-spacing: 5px; color: var(--aura-text); transform: translateY(15px); opacity: 0; transition: 0.5s; text-transform: uppercase;}
