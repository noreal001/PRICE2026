<template>
  <div :class="['bahur-terminal', { 'noir': isDark }]" :style="{ '--p-cols': activePriceCount }" @mousemove="updateLiquidEye">
    
    <div class="liquid-eye" :style="{ transform: `translate(${eyeX}px, ${eyeY}px)` }"></div>
        ...
        <div class="table-frame">
          <div class="sticky-nav-group">
            ...
            <div class="grid-layout-def head no-click">
              <div class="cell id head-txt center border-right">№</div>
              <div class="cell name name-header border-right"><span class="head-txt">Ароматы</span></div>
              <div class="cell desk-only head-txt center border-right">Пол</div>
              <div class="cell desk-only head-txt center border-right">Фабрика</div>
              <div class="cell desk-only head-txt center border-right">Качество</div>
          ...
          <div class="grid-table">
            <div v-for="(p, index) in sortedProducts" :key="p.id + '-' + index" 
              ...
              <div class="row-visual-layer">
                <div class="cell id-zone-square center border-right">
                ...
                <div class="cell name border-right border-right-mobile">
                ...
                <div class="cell desk-only center border-right"><div class="badge-square-matte">{{ getSex(p.gender) }}</div></div>
                <div class="cell desk-only center border-right"><div class="badge-square-matte">{{ p.factory }}</div></div>
                <div class="cell desk-only center border-right"><div class="badge-square-matte">{{ p.quality }}</div></div>
...
const updateLiquidEye = (e) => {
  targetX = e.clientX;
  targetY = e.clientY;
};

const animateEye = () => {
  eyeX.value += (targetX - eyeX.value) * 0.05; // Плавное следование (easing)
  eyeY.value += (targetY - eyeY.value) * 0.05;
  animationFrameId = requestAnimationFrame(animateEye);
};

// Генератор пульсирующей иконки маячка
const setupFavicon = () => {
  ...
  setInterval(() => {
    ...
    const opacity = 0.4 + Math.sin(phase) * 0.4;
    // Черный маячок на белом фоне (или сером, чтобы был виден на любых вкладках)
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
        <rect width="32" height="32" rx="8" fill="#ffffff" />
        <circle cx="16" cy="16" r="${r}" fill="#000000" opacity="${opacity}"/>
...
/* RE-DESIGN: ЧЕТКАЯ КОНТРАСТНОСТЬ И РАЗДЕЛЕНИЕ СЛОЕВ */
.bahur-terminal {
  --bg: #F8F9FA; 
  --text: #000000; 
  --line-main: #000000; 
  --line-dim: rgba(0,0,0,0.3);
  --panel-bg: rgba(255, 255, 255, 0.85); 
  --panel-border: #000000;
  --menu-shadow: 0 20px 50px rgba(0,0,0,0.15);
  
  --dim: #555; --green: #00ff88; --red: #ff3b30;
  --aura-bg: rgba(255,255,255,0.7); --aura-text: #000;
  --sticky-bg: rgba(248, 249, 250, 0.85); 
  --seg-bg: #E9ECEF; --seg-active: #000000; --seg-txt: #6C757D; --seg-txt-active: #ffffff;
...
.noir { 
  --bg: #000000; 
  --text: #ffffff; 
  --line-main: #ffffff; /* Строго белые линии */
  --line-dim: rgba(255,255,255,0.4); /* Четкие отчерченные линии */
  --panel-bg: rgba(15, 15, 15, 0.9); /* Лучшее разделение слоев */
  --panel-border: rgba(255,255,255,0.25);
  --menu-shadow: 0 30px 60px rgba(0,0,0,0.9);
  
  --aura-bg: rgba(0,0,0,0.6); --aura-text: #fff;
  --sticky-bg: rgba(0,0,0,0.85);
  --seg-bg: #1A1A1A; --seg-active: #ffffff; --seg-txt: #888888; --seg-txt-active: #000000;
}

/* LIQUID EYE EFFECT (Fluid Cursor / Aura) */
.liquid-eye {
  position: absolute;
  ...
  pointer-events: none;
  z-index: 0;
  opacity: 0.08; /* Очень тонкий, минималистичный эффект */
  filter: blur(40px);
...
.diagonal-bg {
  ...
  background: repeating-linear-gradient(
    ...
    var(--line-dim) 10px, var(--line-dim) 12px
  );
...
.intro-text { 
  ...
  font-weight: 100; font-size: 70px; color: var(--text); 
...
.error-box-noir { text-align: center; border: 1px solid var(--line-main); padding: 40px 60px; border-radius: 4px; background: var(--bg); max-width: 400px; }
...
.scroll-widget-track::before { content: ''; position: absolute; top: 0; bottom: 0; width: 1px; background: var(--line-dim); }
...
.strip { height: 1px; background: var(--text); width: 100%; position: absolute; left: 0; opacity: 0.5; }
.strip.top { top: 0; } .strip.bottom { bottom: 0; }
.logo-text { font-size: 36px; font-weight: 100; margin: 0; letter-spacing: 0.25em; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; line-height: 1.2; text-shadow: 0 0 10px rgba(255,255,255,0.2); }
...
.header-pill-btn { position: absolute; top: 50%; transform: translateY(-50%); background: transparent; border: 1px solid var(--line-dim); color: var(--text); border-radius: 20px; padding: 6px 12px; font-size: 10px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 5px; transition: 0.2s; z-index: 10; backdrop-filter: blur(10px); }
.header-pill-btn:hover { border-color: var(--text); }
.theme-pos { right: 0; }

/* STICKY NAV */
.sticky-nav-group { position: sticky; top: 0; z-index: 500; background: var(--sticky-bg); backdrop-filter: blur(20px); border-bottom: 1px solid var(--line-main); margin-bottom: 0; }
.controls-luxury { padding: 12px 0; border-bottom: 1px solid var(--line-dim); }

/* БЛОК КНОПОК - РАВНЫЕ РАЗМЕРЫ (FLEX) */
.ctrl-wrapper-desktop { display: flex; justify-content: space-between; align-items: center; width: 100%; gap: 10px; }
.control-item { flex: 1; display: flex; position: relative; }

.main-ctrl-btn { 
  width: 100%; height: 44px; /* Фиксированная высота для одинакового размера */
  background: transparent; border: 1px solid var(--line-dim); color: var(--text); 
...
.main-ctrl-btn:hover { border-color: var(--text); box-shadow: 0 0 10px rgba(255,255,255,0.05); }
.main-ctrl-btn.active-mode { background: var(--text); color: var(--bg); border-color: var(--text); font-weight: 800; }
.main-ctrl-btn.toggle-btn-wrapper.active-mode { background: transparent; color: var(--text); font-weight: 700; }

.ctrl-text-bold { font-weight: 600; }
.btn-txt-fixed { max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.pill-arrow { width: 12px; height: 12px; opacity: 0.8; } 

/* ART TOGGLE */
.art-toggle { width: 30px; height: 18px; border: 1px solid currentColor; border-radius: 99px; position: relative; transition: all 0.3s; display: inline-flex; align-items: center; background: transparent; }
.art-thumb { width: 12px; height: 12px; background: currentColor; border-radius: 50%; position: absolute; left: 2px; transition: all 0.3s; }
.active-mode .art-toggle { background: currentColor; }
.active-mode .art-thumb { background: var(--bg); transform: translateX(12px); }

/* POPUPS (ЛУЧШЕЕ РАЗДЕЛЕНИЕ СЛОЕВ) */
...
.bahur-popup-menu { 
  ...
  backdrop-filter: blur(25px) saturate(120%); /* Эффект стекла 2026 */
  display: flex; flex-direction: column; gap: 10px; z-index: 999; 
}
.bahur-popup-menu.list-mode { left: 0; width: 260px; }
.bahur-popup-menu.filter-mode { right: 0; width: 240px; }

.popup-input { width: 100%; background: var(--seg-bg); border: 1px solid var(--line-dim); padding: 10px; border-radius: 8px; color: var(--text); font-size: 13px; outline: none; margin-bottom: 5px; box-sizing: border-box; }
.popup-input:focus { border-color: var(--text); }
...
.brand-row-btn { display: flex; justify-content: space-between; align-items: center; background: transparent; color: var(--text); border: none; padding: 10px 10px; border-radius: 6px; cursor: pointer; font-size: 12px; text-transform: capitalize; text-align: left; transition: 0.2s; opacity: 0.8; }
.all-brand-btn { opacity: 1; font-weight: 700; margin-bottom: 5px; border-bottom: 1px solid var(--line-dim); padding-bottom: 10px; border-radius: 0; }
.brand-row-btn:hover { background: var(--seg-bg); opacity: 1; }
.brand-font-fix { font-weight: 900; font-size: 11px; opacity: 0.8; }
...
.aroma-sug-brand { font-weight: 900; opacity: 0.5; margin-right: 5px; font-size: 11px; }
.aroma-sug-name { font-weight: 700; font-size: 12px; }

.popup-section { margin-bottom: 8px; }
.popup-label { display: block; font-size: 9px; font-weight: 700; color: var(--text); opacity: 0.7; margin-bottom: 6px; letter-spacing: 1px; text-transform: uppercase; }
.segmented-control { display: flex; background: var(--seg-bg); padding: 4px; border-radius: 8px; border: 1px solid var(--line-dim); }
.segment-btn { flex: 1; background: transparent; border: none; color: var(--seg-txt); padding: 6px 0; font-size: 11px; font-weight: 600; border-radius: 6px; cursor: pointer; transition: 0.2s; }
...
.dash-control-bar { padding-left: 2px; display: flex; justify-content: flex-start; margin-bottom: 10px; }
...
.dash-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 15px; }
.stat-card { border: 1px solid var(--line-main); padding: 20px; background: rgba(0,0,0,0.02); border-left: 4px solid var(--text); backdrop-filter: blur(10px); }
.noir .stat-card { background: rgba(255,255,255,0.02); }
.d-label { display: block; font-size: 9px; font-weight: 700; color: var(--text); opacity: 0.7; margin-bottom: 12px; letter-spacing: 1.5px; text-transform: uppercase; }
.stat-card .v { font-size: 28px; font-weight: 700; }
.q-row-stacked { margin-bottom: 12px; }
.q-meta { display: flex; justify-content: space-between; font-size: 10px; font-weight: 600; margin-bottom: 5px; }
.op-5 { opacity: 0.5; }
.q-track-neon { height: 2px; background: var(--line-dim); width: 100%; overflow: hidden; }
.q-fill-neon { height: 100%; background: var(--text); box-shadow: 0 0 8px var(--text); transition: 1s ease-out; }

.split-top-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; }
.st-item { flex: 1; }
.st-sep { width: 1px; background: var(--line-dim); margin: 0 10px; align-self: stretch; }
.st-price-box { flex: 1.5; }
.avg-price-flex { display: flex; flex-direction: column; gap: 6px; }
.ap-item { font-family: 'JetBrains Mono', monospace; font-size: 12px; color: var(--dim); }
.ap-item .val { color: var(--text); font-weight: 700; font-size: 14px; margin-left: 5px; }

.stock-stack-info { display: flex; flex-direction: column; margin-top: 12px; gap: 4px; }
...
.stock-left { flex-shrink: 0; margin-right: 20px; }
.stock-big-num { font-size: 38px; font-weight: 700; line-height: 1; }
.stock-right { flex-grow: 1; display: flex; flex-direction: column; justify-content: center; }
.q-track-neon-thick { height: 8px; background: var(--line-dim); width: 100%; border-radius: 4px; overflow: hidden; margin-bottom: 8px; }
...
.top-switch-btn-subtle { background: transparent; border: 1px solid var(--line-dim); color: var(--text); padding: 8px 16px; border-radius: 20px; font-size: 11px; font-weight: 700; cursor: pointer; opacity: 0.8; transition: 0.2s; display: flex; align-items: center; gap: 8px; }
.top-switch-btn-subtle:hover { border-color: var(--text); opacity: 1; }
.btn-subtle-label { color: var(--dim); }

.top-list-scroll-container { max-height: 80px; overflow-y: auto; display: flex; flex-direction: column; gap: 6px; }
.top-row-compact { display: flex; justify-content: space-between; align-items: center; font-size: 11px; padding: 4px 0; border-bottom: 1px solid var(--line-dim); }
.tr-left-main { display: flex; align-items: center; width: 40%; overflow: hidden; }
.top-num { color: var(--dim); margin-right: 6px; font-size: 10px; min-width: 14px; font-weight: 700; }
.top-name { overflow: hidden; white-space: nowrap; text-overflow: ellipsis; font-weight: 600; font-size: 11px; }
.tr-mid-graph { flex-grow: 1; margin: 0 12px; display: flex; align-items: center; }
.mini-bar-track { width: 100%; height: 3px; background: var(--line-dim); overflow: hidden; border-radius: 2px; }
.mini-bar-fill { height: 100%; background: var(--text); }
.tr-right-meta { display: flex; align-items: center; gap: 6px; }
.badge-mini { border: 1px solid var(--line-dim); padding: 2px 4px; font-size: 9px; color: var(--text); font-weight: 600; border-radius: 3px; }
.top-val { font-weight: 800; margin-left: 6px; font-size: 10px; min-width: 30px; text-align: right; }

/* ТАБЛИЦА - ЧЕТКИЕ ЛИНИИ */
.grid-table { display: flex; flex-direction: column; width: 100%; min-width: 1000px; border: 1px solid var(--line-main); border-top: none; }
.grid-layout-def { 
  display: grid; 
...
.grid-layout-def:not(.head) { background: transparent; border-bottom: 1px solid var(--line-main); position: relative; overflow: hidden; }

/* Все ячейки имеют четкие обводки (Border Right) */
.cell { 
  height: auto; display: flex; align-items: center; padding: 15px; 
  box-sizing: border-box; overflow: hidden; 
}
.border-right { border-right: 1px solid var(--line-main); }

.head-txt { font-size: 9px; font-weight: 800; color: var(--text); text-transform: uppercase; letter-spacing: 1.5px; font-family: 'Helvetica Neue', sans-serif;}
.center { justify-content: center; text-align: center; }
.row-visual-layer { display: contents; }
.clickable-row { cursor: pointer; transition: 0.3s; }
.out { opacity: 0.4; }
.badge-new-box { background: var(--text); color: var(--bg); font-size: 9px; font-weight: 900; padding: 2px 4px; border-radius: 2px; margin: 4px auto 0; display: inline-block; }
.desk-only-new { display: inline-block; }
.lamp-sq { width: 16px; height: 4px; margin: 0 auto; border-radius: 2px; transition: 0.3s; }
.lamp-sq.green { background: #00ff41; box-shadow: 0 0 10px #00ff41; }
.lamp-sq.red { background: #ff2a2a; box-shadow: 0 0 10px #ff2a2a; }
.lamp-sq.standard { background: var(--text); }
.badge-square-matte { font-size: 10px; font-weight: 800; border: 1px solid var(--line-dim); padding: 6px 14px; letter-spacing: 1px; text-transform: uppercase; margin: 0 auto; color: var(--text); font-family: 'Kollektif', sans-serif;}
.m-square-matte { font-size: 9px; font-weight: 800; border: 1px solid var(--line-dim); padding: 3px 8px; text-transform: uppercase; }
.id-zone-square { flex-direction: column; gap: 6px; justify-content: center; padding: 10px 0; }
.id-sq-top { font-size: 16px; font-weight: 900; color: var(--text); opacity: 0.8; }
.scent-info { width: 100%; padding-left: 15px; }
.brand-code { font-size: 11px; font-weight: 800; opacity: 0.6; display: block; text-transform: uppercase; letter-spacing: 1.5px; }
.scent-title { font-size: 18px; font-weight: 600; line-height: 1.3; letter-spacing: 0.5px; }
.mobile-only-meta { display: none; margin-top: 10px; gap: 6px; align-items: center; }
...
.price-section { display: grid; height: 100%; width: 100%; align-items: stretch; transition: all 0.4s; }
.p-col { 
  text-align: center; font-size: 16px; height: auto; 
  display: flex; align-items: center; justify-content: center; 
  overflow: hidden; white-space: nowrap; box-sizing: border-box; 
}
.p-col.line { border-right: 1px solid var(--line-main); } 
.head-p .p-col { font-size: 9px; font-weight: 800; color: var(--text); letter-spacing: 1.5px; text-transform: uppercase; }

/* AURA HOVER EFFECT (Liquid Vibe) */
...
.clickable-row:hover .row-aura-overlay, .clickable-row.simulated-hover .row-aura-overlay { opacity: 1; backdrop-filter: blur(12px) saturate(150%); background: var(--aura-bg); }
.clickable-row:active .row-aura-overlay { opacity: 0 !important; backdrop-filter: none !important; }
.aura-text { font-size: 13px; font-weight: 900; letter-spacing: 6px; color: var(--aura-text); transform: translateY(20px); opacity: 0; transition: 0.5s; font-family: 'Kollektif', sans-serif; text-shadow: 0 4px 10px rgba(0,0,0,0.2); }
.clickable-row:hover .aura-text, .clickable-row.simulated-hover .aura-text { opacity: 1; transform: translateY(0); }

@media (max-width: 900px) {
  .dash-grid { grid-template-columns: 1fr 1fr; }
  .span-full { grid-column: span 2; } 
  .grid-table { min-width: 100%; border: none; border-top: 1px solid var(--line-main); }
  .desk-only { display: none; }
  .mobile-only-meta { display: flex; }
  
  .grid-layout-def { grid-template-columns: 40px 1fr calc(var(--p-cols) * 65px); }
  .border-right { border-right: none; } /* Убираем правые рамки на мобильных кроме нужных */
  .id-zone-square { padding: 4px 0 !important; align-items: center; justify-content: flex-start; padding-top: 15px !important; border-right: 1px solid var(--line-main); }
  .id-sq-top { font-size: 12px; margin-bottom: 6px; }
  .lamp-sq { width: 8px; height: 8px; border-radius: 50%; }
  
  .border-right-mobile { border-right: 1px solid var(--line-main) !important; padding-right: 10px; }
  .scent-info { padding-left: 5px; }
  .scent-title { font-size: 15px; }
  
  .price-container { width: calc(var(--p-cols) * 65px); display: flex; flex-direction: column; height: auto; }
  .price-section { height: 100%; }
  .p-col { padding: 0; font-size: 13px; border-right: none !important; border-bottom: 1px solid var(--line-main); height: 100%; }
  .p-col.line { border-bottom: 1px solid var(--line-main) !important; border-right: none !important; }
  .p-col.last { border-bottom: none !important; }
  .head-p { display: none; } /* Скрываем хидер цен на мобилках если он вертикальный */
  
  .aura-text { font-size: 10px; letter-spacing: 3px; }
  
  .ctrl-wrapper-desktop { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
  .control-item { min-width: 0; width: 100%; }
  .main-ctrl-btn { font-size: 13px; padding: 0; height: 48px; font-weight: 800; border-radius: 12px; }
  .btn-txt-fixed { max-width: 100px; }
  
  .desk-only-new { display: none; }
  .bahur-popup-menu.list-mode, .bahur-popup-menu.filter-mode { left: 0 !important; right: 0 !important; margin: auto !important; width: 90vw !important; top: 60px; }
