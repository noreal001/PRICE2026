<template>
  <div :class="['bahur-terminal', { 'noir': isDark }]" :style="{ '--p-cols': activePriceCount }">
      ...
      <div v-else>
        ...
        <div class="dash-control-bar">
           <button @click="showDash = !showDash" class="header-icon-btn stats-pos" title="Статистика">
             <svg :class="['h-icon', { 'closed-arrow': !showDash }]" viewBox="0 0 24 24">
            ...
            <div class="dash-grid">
              ...
              <div class="stat-card">
                ...
                <div class="mini-stat-row">
                    <span>Есть: {{ stats.countAvail }}</span>
                    <span class="dim-slash">|</span>
                    <span>Нет: {{ stats.countOut }}</span>
                </div>
              </div>
              ...
              <div class="stat-card">
                ...
                <div class="stock-box-stacked">
                  ...
                  <div class="q-track-neon">
                    <div class="q-fill-neon white-part" :style="{ width: stats.availability + '%' }"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="stat-card relative-zone">
                <div class="top-row-flex">
                  <label class="d-label">ТОП: {{ statsMode === '6m' ? '6 МЕС' : 'ВСЕ' }}</label>
                  <button @click="toggleStatsMode" class="mini-arrow-btn mode-switch" title="Переключить период">⇄</button>
                </div>
                
                <div class="top-list-scroll-container custom-scroll-minimal">
                   <div v-for="(item, idx) in stats.topListFull" :key="idx" class="top-row">
                      <div class="top-name-box">
                         <span class="top-num mono">{{ idx + 1 }}.</span>
                         <span class="top-name rus-font">{{ item.name }}</span>
                      </div>
        ...
        <div class="table-frame">
          <div class="sticky-nav-group">
            ...
            <section class="controls-luxury relative-zone">
              ...
              <div class="top-bar-controls-grid-4">
                   ...
                   <transition name="pop">
                      <div v-if="showBrandMenu" class="bahur-popup-menu list-mode">
                         <div class="search-input-box">
                            <input ref="brandInputRef" v-model="tempBrandInput" type="text" placeholder="Поиск бренда..." class="popup-input rus-font" />
                         </div>
                            ...
                            <div class="brands-list-vertical">
                              <button @click="clearBrands" class="brand-row-btn rus-font all-brand-btn">
                                <div class="brand-left-group">
                                  <svg class="circle-check-icon" viewBox="0 0 24 24">
                              ...
                              <button v-for="b in filteredBrandsDropdown" :key="b" @click="toggleBrandSelection(b)" class="brand-row-btn eng-font">
                                <div class="brand-left-group">
                                  <span class="brand-txt-truncate">{{ b }}</span>
                                </div>
                                <svg v-if="selectedBrands.includes(b)" class="check-status" viewBox="0 0 24 24">
                ...
                <div class="control-item relative-zone">
                   ...
                   <transition name="pop">
                      <div v-if="showAromaMenu" class="bahur-popup-menu list-mode">
                         <div class="search-input-box">
                            <input ref="aromaInputRef" v-model="tempAromaInput" type="text" placeholder="Поиск аромата..." class="popup-input rus-font" />
                         </div>
                         <div class="brands-scroll-area custom-scroll-minimal">
                            <div class="brands-list-vertical">
                              ...
                              <button v-for="(item, index) in aromaSuggestions" :key="index" @click="toggleAromaSelection(item.name)" class="brand-row-btn">
                                ...
                                <svg v-if="selectedAromas.includes(item.name)" class="check-status" viewBox="0 0 24 24"><path fill="currentColor" d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z" /></svg>
                              </button>
                ...
                <div class="control-item">
                  <button @click="toggleNew" :class="['main-ctrl-btn', { 'active': isNewOnly }]">
                    <span class="rus-font">Новинки</span>
                  </button>
                </div>
            ...
            <div class="grid-layout-def head no-click">
              ...
              <div class="cell name name-header">
                <span class="head-txt">Ароматы</span>
              </div>
          ...
          <div v-if="loading" class="loading-container">
             <div :class="['spiral-loader', loaderColor]"></div>
          </div>
    ...
    <div class="scroll-widget-noir">
       <button class="s-btn up" @mousedown="startScroll('up')" @touchstart.passive="startScroll('up')" @mouseup="stopScroll" @mouseleave="stopScroll" @touchend="stopScroll" @click="scrollPage('top')">
       ...
       <button class="s-btn down" @mousedown="startScroll('down')" @touchstart.passive="startScroll('down')" @mouseup="stopScroll" @mouseleave="stopScroll" @touchend="stopScroll" @click="scrollPage('bottom')">
...
const startScroll = (dir) => {
  stopScroll();
  const step = dir === 'up' ? -30 : 30; // Speed of hold-scroll
...
// --- State for Statistics Mode ---
const statsMode = ref('6m'); // '6m' or 'all'

const toggleStatsMode = () => {
  statsMode.value = statsMode.value === '6m' ? 'all' : '6m';
}
...
const toggleAromaMenu = () => {
  ...
  closeAllMenus();
  showAromaMenu.value = true;
  tempAromaInput.value = '';
  nextTick(() => aromaInputRef.value?.focus());
}
...
const brandLabel = computed(() => {
  ...
  if (len === 1) return selectedBrands.value[0];
  return `${len} Бренда`;
});
...
const aromaLabel = computed(() => {
  ...
  if (len === 1) return selectedAromas.value[0];
  return `${len} Аромата`;
});
...
const loadData = async () => {
  ...
  loaderColor.value = colors[Math.floor(Math.random() * colors.length)];
  errorMsg.value = null;
  try {
    ...
    startAutoHighlight();
  } catch (e) { console.error(e); errorMsg.value = "Не удалось подключиться к базе данных. Проверьте соединение."; } finally { setTimeout(() => loading.value = false, 500); }
}
...
const uniqueBrands = computed(() => {
  ...
  products.value.forEach(p => { if(p.brand) brands.add(p.brand); });
  return Array.from(brands).sort();
});

const filteredBrandsDropdown = computed(() => {
  const s = tempBrandInput.value.toLowerCase(); 
  if (!s) return uniqueBrands.value;
  return uniqueBrands.value.filter(b => b.toLowerCase().includes(s));
});

const aromaSuggestions = computed(() => {
  const s = tempAromaInput.value.toLowerCase();
  let list = products.value;
  if (s) {
    list = list.filter(p => p.brand.toLowerCase().includes(s) || p.name.toLowerCase().includes(s));
  }
  return list.map(p => ({ brand: p.brand, name: p.name })).slice(0, 50);
});
...
const stats = computed(() => {
  ...
  p.forEach(i => {
    ...
    if (f.includes('LUZI')) factories['LUZI']++;
    else if (f.includes('EPS')) factories['EPS']++;
    else if (f.includes('SELUZ')) factories['SELUZ']++;
  });
  
  const topListFull = [...p].sort((a,b) => {
      const valA = statsMode.value === '6m' ? a.sales6m : a.salesAll;
      const valB = statsMode.value === '6m' ? b.sales6m : b.salesAll;
      return valB - valA;
  });
...
@font-face {
    ...
    src: local('Kollektif'), url('https://fonts.cdnfonts.com/s/16912/Kollektif.woff') format('woff');
    font-weight: normal;
    font-style: normal;
}
...
/* CUSTOM SCROLLBARS (GLOBAL PAGE/TABLE) */
...
.retry-btn-noir { 
  background: var(--text); border: none; color: var(--bg); padding: 12px 24px; 
...
.scroll-widget-noir:hover { opacity: 1; }
.s-btn {
...
.s-track {
  width: 100%; height: 120px; /* Compact height */
  display: flex; flex-direction: column; justify-content: space-between; align-items: center;
  padding: 5px 0;
  position: relative;
}
.s-mark {
  width: 12px; height: 1px; background: var(--text); opacity: 0.5; cursor: pointer;
  transition: 0.2s;
}
...
.main-ctrl-btn {
  width: 100%; background: var(--btn-ctrl-bg); border: none; color: var(--text);
  padding: 10px 0; border-radius: 30px; font-size: 10px; font-weight: 700;
  cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 4px; 
  transition: all 0.2s; letter-spacing: 0.5px; white-space: nowrap; text-transform: none; 
}
...
.bahur-popup-menu {
  position: absolute; top: 50px; background: var(--panel-bg); border: 0.5px solid var(--border); 
  border-radius: 16px; padding: 15px; box-shadow: 0 30px 60px rgba(0,0,0,0.95);
...
.top-list-scroll-container { max-height: 85px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; padding-right: 2px; }
.top-row { display: flex; justify-content: space-between; align-items: center; font-size: 10px; }
.top-name-box { display: flex; align-items: center; overflow: hidden; max-width: 75%; }
.top-num { color: var(--dim); margin-right: 6px; font-size: 9px; min-width: 15px; }
.top-name { overflow: hidden; white-space: nowrap; text-overflow: ellipsis; font-weight: 500; }
.top-val { font-weight: 700; }
...
.spiral-loader { width: 40px; height: 40px; border-radius: 50%; border: 2px solid transparent; animation: spin 1s linear infinite; }
.spiral-loader.red { border-top-color: #ff3b30; border-right-color: rgba(255,59,48,0.5); }
.spiral-loader.blue { border-top-color: #007aff; border-right-color: rgba(0,122,255,0.5); }
.spiral-loader.purple { border-top-color: #bf5af2; border-right-color: rgba(191,90,242,0.5); }
