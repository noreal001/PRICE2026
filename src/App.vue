              ...
              <div class="row-visual-layer">
                ...
                <div class="cell id-zone-square center">
                  ...
                  <div class="status-symbol mono">
                    <span v-if="p.isOut" class="status-btn-pill out-pill">-</span>
                    <span v-else-if="p.hasPlus" class="status-btn-pill plus-pill">+</span>
                    <span v-else-if="p.hasStar" class="status-btn-pill star-pill">*</span>
                  </div>
                </div>
...
/* ЦВЕТА КАК НА СКРИНШОТЕ (ФОН ТЕМНЫЙ, КАРТОЧКИ И ЦЕНЫ СВОИ) */
.bahur-terminal {
  --bg: #0d0d0f; /* Самый темный фон страницы */
  --text: #fff; --border: rgba(255,255,255,0.06); --dim: #888; 
  --panel-bg: #1c1c1e; /* Меню */
  
  --card-bg: transparent; /* Строка теперь без фона */
  --inner-pill-bg: #1a1a1c; /* Светлее фона (для названий и тегов) */
  --inner-pill-dark: #050505; /* Практически черный (для цен) */
...
.noir { 
  --bg: #0d0d0f;
  --text: #fff; --aura-bg: rgba(0,0,0,0.4); --border: rgba(255,255,255,0.05);
  --sticky-bg: rgba(13, 13, 15, 0.95); --panel-bg: #1c1c1e; 
  --card-bg: transparent; 
  --inner-pill-bg: #1a1a1c; --inner-pill-dark: #050505;
...
.bahur-terminal:not(.noir) { 
  ...
  --panel-bg: #ffffff; 
  --card-bg: transparent; 
  --inner-pill-bg: #ffffff; --inner-pill-dark: #e5e5ea;
...
.error-box-noir { text-align: center; border: 1px solid var(--border); padding: 40px 60px; border-radius: 16px; background: var(--panel-bg); }
...
.retry-btn-noir { background: var(--text); border: none; color: var(--bg); padding: 12px 24px; font-family: 'JetBrains Mono', monospace; font-size: 11px; cursor: pointer; transition: 0.3s; text-transform: uppercase; font-weight: 700; border-radius: 20px;}
...
.stat-card { border: 1px solid var(--border); padding: 18px; background: var(--panel-bg); border-radius: 16px; }
...
.ap-item { font-size: 12px; color: var(--dim); font-weight: 600;}
...
.ss-row span { color: var(--text); margin-left: 4px; font-weight: 800;}
...
.top-num { color: var(--dim); font-weight: 800; margin-right: 5px; }
...
.badge-mini { border: 1px solid var(--border); padding: 1px 4px; font-size: 9px; font-weight: 800; color: var(--text); border-radius: 4px; }
.top-val { font-weight: 800; margin-left: 4px; min-width: 25px; text-align: right; }

/* TABLE HEADER */
.grid-layout-def.head { 
  display: grid; 
  /* Сетка из твоего кода */
...
/* СТИЛИ ИНТЕГРИРОВАННОГО ПОИСКА В ЗАГОЛОВОК */
...
/* СЕТКА АРОМАТОВ (АБСОЛЮТНО ПРОЗРАЧНАЯ ОСНОВА, КАК НА СКРИНШОТЕ) */
.grid-table { 
  display: flex; flex-direction: column; gap: 8px; /* Расстояние по вертикали между строками */
  width: 100%; min-width: 1000px; border: none; padding-top: 12px;
}
...
.grid-layout-def:not(.head) { 
  ...
  grid-template-columns: 60px 1fr 80px 120px 120px calc(var(--p-cols) * 80px); 
  align-items: stretch; box-sizing: border-box; width: 100%;
  background: var(--card-bg); /* Полностью прозрачно */
  border: none;
  position: relative; 
  transition: all 0.25s cubic-bezier(0.25, 0.8, 0.25, 1);
}

/* ЭФФЕКТ ПРИ НАВЕДЕНИИ (Сами плашки внутри светлеют) */
.grid-layout-def.clickable-row:hover .inner-pill-main,
.grid-layout-def.clickable-row.simulated-hover .inner-pill-main,
.grid-layout-def.clickable-row:hover .inner-pill-badge,
.grid-layout-def.clickable-row.simulated-hover .inner-pill-badge,
.grid-layout-def.clickable-row:hover .inner-pill-price,
.grid-layout-def.clickable-row.simulated-hover .inner-pill-price {
  background: rgba(255, 255, 255, 0.15); /* Тактильное свечение */
}

/* ЯЧЕЙКИ В ПРОЗРАЧНОМ РЯДУ */
.cell { 
  height: auto; display: flex; align-items: center; padding: 4px 6px; 
...
/* СВОБОДНЫЕ ID И СТАТУСЫ СЛЕВА (КАК НА СКРИНШОТЕ) */
.id-zone-square { flex-direction: column; justify-content: center; padding-left: 10px;}
.id-sq-top { font-size: 16px; font-weight: 900; color: var(--dim); line-height: 1; margin-bottom: 6px; }
...
/* ПЛАШКИ (PILLS) ДЛЯ АРОМАТА, ТЕГОВ И ЦЕН (КАК НА СКРИНШОТЕ) */
.inner-pill-main {
  background: var(--inner-pill-bg);
  border-radius: 16px;
  padding: 10px 16px; border: 1px solid rgba(255,255,255,0.02);
...
.inner-pill-badge {
  ...
  padding: 8px 12px;
  font-weight: 800;
  font-size: 10px; /* Мелкий, жирный шрифт */
  color: var(--text);
...
/* ГЛУБОКИЕ ЧЕРНЫЕ КНОПКИ ДЛЯ ЦЕН (Эффект "наоборот") */
.inner-pill-price {
...
.scent-title { font-weight: 800; font-size: 17px; line-height: 1.2; letter-spacing: 0.5px; margin-top: 2px;}

.mobile-only-meta { display: none; margin-top: 8px; gap: 5px; align-items: center; }

/* ЦЕНЫ В СВОИХ ЯЧЕЙКАХ */
.price-container { width: calc(var(--p-cols) * 80px); display: flex; align-items: center; }
.price-section { display: grid; gap: 8px; width: 100%; padding: 4px;}
...
@media (max-width: 900px) {
  ...
  /* МОБИЛЬНАЯ ВЕРСИЯ: Увеличиваем отступы между рядами */
  .grid-table { min-width: 100%; border: none; gap: 16px; padding-top: 12px;}
  .cell { border-right: none; }
  .desk-only { display: none; }
  .mobile-only-meta { display: flex; }
  
  /* СОХРАНЯЕМ ОРИГИНАЛЬНУЮ СЕТКУ ДЛЯ МОБИЛКИ */
  ...
  .id-sq-top { font-size: 13px; margin-bottom: 6px; }
  .status-btn-pill { width: 18px; height: 18px; font-size: 12px; border-radius: 6px;}
  
  .inner-pill-main { padding: 12px 10px; border-radius: 16px;}
  .scent-title { font-size: 14px; }
  
  .price-container { width: calc(var(--p-cols) * 55px); display: flex; flex-direction: column; justify-content: center;}
  
  /* ЦЕНЫ В МОБИЛКЕ - Строго твоя сетка с добавленным воздухом */
  .price-section { display: flex; flex-direction: column; gap: 6px; padding: 0; width: 100%;}
  .inner-pill-price { padding: 10px 4px; font-size: 12px; border-radius: 12px;}
